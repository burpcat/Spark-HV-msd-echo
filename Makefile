...
app.name=item similarity
jar.name=item-sim.jar
maven.jar.name=item-similarity-1.0.jar
job.name=sim.SparseItemSim
local.master=local[*]
local.input=input
local.output=output
local.topk=10
...
aws.bucket.name=cs6240-project-msd
aws.input=input
aws.output=output
aws.log.dir=log
...
# Add Hadoop classpath to Spark
hadoop.classpath=$(shell ${hadoop.root}/bin/hadoop classpath)
...
# 	/usr/local/spark-3.3.2-bin-without-hadoop/bin/spark-submit --class ${job.name} --master ${local.master} --name "${app.name}" ${jar.name} ${local.input} ${local.output}
local: jar clean-local-output
	${spark.root}/bin/spark-submit \
		--class ${job.name} \
		--master ${local.master} \
		--name "${app.name}" \
		--conf spark.driver.extraClassPath=${jar.name}:${hadoop.classpath} \
		--conf spark.executor.extraClassPath=${jar.name}:${hadoop.classpath} \
		${jar.name} ${local.input} ${local.output} ${local.topk}
# Start HDFS
start-hdfs:
...
aws: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
		...
		--steps Type=CUSTOM_JAR,Name="${app.name}",Jar="command-runner.jar",ActionOnFailure=TERMINATE_CLUSTER,Args=["spark-submit","--deploy-mode","cluster","--class","${job.name}","s3://${aws.bucket.name}/${jar.name}","s3://${aws.bucket.name}/${aws.input}","s3://${aws.bucket.name}/${aws.output}","${local.topk}"] \
